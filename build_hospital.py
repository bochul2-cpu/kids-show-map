"""아이랑 병원 페이지(hospital.html)를 생성한다. "아이가 아파요" 같은 급한 상황에
쓰는 페이지라 지도 없이 목록만 빠르게 뜨도록 만든다(네이버 지도 API 로딩 자체를
건너뛴다). 소아과는 진료시간 기준으로 "지금 열려있는 곳만" 보여주고, 응급실은
항상 전체를 보여준다(전화로 확인하고 가는 게 맞는 영역이라 필터링하지 않는다).
"""
from settings import GA_MEASUREMENT_ID
from contact_widget import render_contact_widget

HOSPITAL_OUTPUT_PATH = "hospital.html"

HTML_TEMPLATE = """<!DOCTYPE html>
<html lang="ko">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>아이랑 병원 - 내 주변 소아과/응급실 찾기</title>
<meta name="description" content="아이가 아파요? 내 위치 기준 20km, 지금 열려있는 소아과·응급실을 바로 찾아보세요">
<link rel="manifest" href="manifest.json">
<meta name="theme-color" content="#e0507a">
<link rel="icon" href="icons/icon-192.png">
<meta name="apple-mobile-web-app-capable" content="yes">
<meta name="apple-mobile-web-app-status-bar-style" content="black-translucent">
<meta name="apple-mobile-web-app-title" content="아이랑 병원">
<link rel="apple-touch-icon" href="icons/apple-touch-icon.png">
<meta property="og:type" content="website">
<meta property="og:title" content="아이랑 병원">
<meta property="og:description" content="아이가 아파요? 내 위치 기준 20km, 지금 열려있는 소아과·응급실을 바로 찾아보세요">
<meta property="og:image" content="https://bucheonkids.com/icons/og-image.png">
<meta property="og:url" content="https://bucheonkids.com/hospital.html">
<meta name="twitter:card" content="summary_large_image">
<script async src="https://www.googletagmanager.com/gtag/js?id={ga_measurement_id}"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){{ dataLayer.push(arguments); }}
  gtag('js', new Date());
  gtag('config', '{ga_measurement_id}');
</script>
<style>
  * {{ box-sizing: border-box; }}
  html, body {{
    margin: 0; font-family: -apple-system, "Malgun Gothic", sans-serif;
    background: #fff5f7;
  }}
  .topbar {{
    background: white; box-shadow: 0 2px 8px rgba(0,0,0,0.08); padding: 12px 16px;
  }}
  .topbar-header {{ display: flex; align-items: center; justify-content: space-between; margin-bottom: 8px; }}
  .brand {{ display: flex; align-items: center; gap: 6px; }}
  .brand .logo {{ font-size: 18px; }}
  .brand h1 {{ font-size: 15px; margin: 0; color: #333; font-weight: 800; }}
  .header-actions {{ display: flex; align-items: center; gap: 8px; }}
  .nav-link, .contact-link {{
    flex-shrink: 0; font-size: 12px; text-decoration: none; font-weight: 700;
    border-radius: 14px; padding: 5px 11px; white-space: nowrap;
  }}
  .nav-link {{ color: #666; border: 1.5px solid #eee; }}
  .contact-link {{ color: #e0507a; border: 1.5px solid #ffcbd8; }}
  .nav-link:hover {{ background: #f8f8f8; }}
  .contact-link:hover {{ background: #fff0f4; }}

  .tagline {{ font-size: 12px; color: #999; margin: 0 0 10px; }}

  .tab-row {{ display: flex; gap: 6px; margin-bottom: 8px; }}
  .tab-btn {{
    flex: 1; border: 1.5px solid #eee; background: white; color: #666;
    border-radius: 10px; padding: 10px; font-size: 13.5px; font-weight: 700; cursor: pointer;
  }}
  .tab-btn.active {{ background: #e0507a; border-color: #e0507a; color: white; }}

  .locate-row {{ display: flex; align-items: center; gap: 8px; }}
  .locate-btn {{
    border: 1.5px solid #ffcbd8; background: white; color: #e0507a;
    border-radius: 10px; padding: 6px 12px; font-size: 12.5px; cursor: pointer; white-space: nowrap;
  }}
  .locate-status {{ font-size: 11.5px; color: #999; }}

  .list-items {{ padding: 10px 12px 40px; max-width: 640px; margin: 0 auto; }}
  .empty-msg {{ text-align: center; color: #bbb; font-size: 13.5px; padding: 60px 20px; line-height: 1.6; }}

  .h-card {{
    background: white; border-radius: 12px; padding: 14px; margin-bottom: 10px;
    box-shadow: 0 1px 4px rgba(0,0,0,0.06);
  }}
  .h-card .badge {{
    display: inline-block; font-size: 10.5px; font-weight: 700; padding: 2px 8px; border-radius: 8px;
    margin-bottom: 6px;
  }}
  .badge.open {{ background: #e6f7ec; color: #1a8a4a; }}
  .badge.general {{ background: #fff4e0; color: #b26a00; }}
  .badge.er {{ background: #fdeaea; color: #c0392b; }}
  .h-card h4 {{ font-size: 15px; margin: 0 0 4px; }}
  .h-card h4 a {{ color: #333; text-decoration: none; }}
  .h-card h4 a:hover {{ text-decoration: underline; }}
  .h-card h4 .search-hint {{ font-size: 11px; color: #bbb; font-weight: 400; margin-left: 4px; }}
  .h-card .meta {{ font-size: 12.5px; color: #888; margin-bottom: 2px; }}
  .h-card .dist {{ font-size: 11.5px; color: #bbb; }}
  .h-card .btn-row {{ display: flex; gap: 6px; margin-top: 10px; }}
  .h-card .btn-row a {{
    flex: 1; text-align: center; padding: 9px 0; border-radius: 8px; text-decoration: none;
    font-size: 13px; font-weight: 700;
  }}
  .call-btn {{ background: #e0507a; color: white; }}
  .directions-btn {{ background: #fff0f4; color: #c0392b; }}
  .lang-toggle {{
    flex-shrink: 0; font-size: 12px; color: #666; background: white; font-weight: 700;
    border: 1.5px solid #eee; border-radius: 14px; padding: 5px 10px; cursor: pointer; white-space: nowrap;
  }}
  .lang-toggle:hover {{ background: #f8f8f8; }}
  {contact_widget_css}
</style>
</head>
<body>
<div class="topbar">
  <div class="topbar-header">
    <div class="brand"><span class="logo">🏥</span><h1>아이랑 병원</h1></div>
    <div class="header-actions">
      <a class="nav-link" id="mapLink" href="index.html">🧸 가볼까</a>
      <button type="button" class="contact-link" id="contactOpenBtn">💬 문의하기</button>
      <button type="button" class="lang-toggle" id="langToggle">EN</button>
    </div>
  </div>
  <p class="tagline" id="taglineText">아이가 아파요? 내 위치 기준 20km, 지금 갈 수 있는 곳만 보여드려요</p>
  <div class="tab-row">
    <button type="button" class="tab-btn active" data-cat="소아과" id="tabClinic">🧒 병원<br>(지금 열린 곳)</button>
    <button type="button" class="tab-btn" data-cat="응급실" id="tabEr">🚑 응급실</button>
    <button type="button" class="tab-btn" data-cat="약국" id="tabPharmacy">💊 약국</button>
  </div>
  <div class="locate-row">
    <button type="button" class="locate-btn" id="locateBtn">📍 내 위치 다시 확인</button>
    <span class="locate-status" id="locateStatus"></span>
  </div>
</div>
<div class="list-items" id="listItems"></div>
<script>
  const DEFAULT_CENTER = [37.5034, 126.7660]; // 부천시청 - GPS 거부/실패 시 기본값
  const FIXED_RADIUS_KM = 20;
  let refPoint = DEFAULT_CENTER;
  let allHospitals = [];
  let activeCat = '소아과';

  // 화면 UI 문구만 번역 대상이다. 병원/약국 이름·주소는 데이터 그대로 한국어로 둔다.
  const T = {{
    ko: {{
      mapLink: '🧸 가볼까',
      contactBtn: '💬 문의하기',
      tagline: '아이가 아파요? 내 위치 기준 20km, 지금 갈 수 있는 곳만 보여드려요',
      tabClinic: '🧒 병원<br>(지금 열린 곳)',
      tabEr: '🚑 응급실',
      tabPharmacy: '💊 약국',
      locateBtn: '📍 내 위치 다시 확인',
      locating: '위치 확인 중...',
      noGeoSupport: '위치 기능을 지원하지 않는 브라우저예요',
      gotLocation: '내 위치 기준으로 보여드려요',
      deniedLocation: '위치 정보를 가져올 수 없어요 (기본 지역 기준으로 보여드려요)',
      searchHint: '🔍 후기 검색',
      callBtn: '📞 전화하기',
      directionsBtn: '🚗 길찾기',
      todayHoursNone: '오늘 진료시간 정보 없음',
      todayHoursPrefix: '오늘',
      badgeClinic: h => `🟢 소아과 진료중 · ${{h}}`,
      badgeGeneral: h => `⚠️ 진료중(소아과 아님) · ${{h}} · 전화 확인 필요`,
      badgePharmacy: h => `💊 영업중 · ${{h}}`,
      badgeEr: '🚑 응급실',
      emptyClinic: '지금 문 연 병원이 없어요 😢<br>응급실 탭을 확인해보세요',
      emptyPharmacy: '지금 문 연 약국이 없어요 😢',
      emptyEr: '주변에 응급실 정보가 없어요',
      emptyDefault: '결과가 없어요',
      loadFailed: '데이터를 불러오지 못했습니다',
    }},
    en: {{
      mapLink: '🧸 Outings',
      contactBtn: '💬 Contact',
      tagline: 'Is your child sick? Places within 20km of your location that are open now',
      tabClinic: '🧒 Clinic<br>(Open now)',
      tabEr: '🚑 ER',
      tabPharmacy: '💊 Pharmacy',
      locateBtn: '📍 Refresh my location',
      locating: 'Checking location...',
      noGeoSupport: 'This browser does not support location',
      gotLocation: 'Showing results near your location',
      deniedLocation: 'Could not get your location (showing default area)',
      searchHint: '🔍 Search reviews',
      callBtn: '📞 Call',
      directionsBtn: '🚗 Directions',
      todayHoursNone: 'No hours info for today',
      todayHoursPrefix: 'Today',
      badgeClinic: h => `🟢 Open now (pediatric) · ${{h}}`,
      badgeGeneral: h => `⚠️ Open (not pediatric) · ${{h}} · call to confirm`,
      badgePharmacy: h => `💊 Open now · ${{h}}`,
      badgeEr: '🚑 ER',
      emptyClinic: 'No clinics open right now 😢<br>Check the ER tab',
      emptyPharmacy: 'No pharmacies open right now 😢',
      emptyEr: 'No ER info nearby',
      emptyDefault: 'No results',
      loadFailed: 'Failed to load data',
    }},
  }};

  let lang = localStorage.getItem('lang') || 'ko';
  function t(key) {{ return T[lang][key]; }}

  function applyStaticText() {{
    document.getElementById('mapLink').textContent = t('mapLink');
    document.getElementById('contactOpenBtn').textContent = t('contactBtn');
    document.getElementById('taglineText').textContent = t('tagline');
    document.getElementById('tabClinic').innerHTML = t('tabClinic');
    document.getElementById('tabEr').textContent = t('tabEr');
    document.getElementById('tabPharmacy').textContent = t('tabPharmacy');
    document.getElementById('locateBtn').textContent = t('locateBtn');
    document.getElementById('langToggle').textContent = lang === 'ko' ? 'EN' : '한글';
    document.documentElement.lang = lang;
  }}

  document.getElementById('langToggle').addEventListener('click', () => {{
    lang = lang === 'ko' ? 'en' : 'ko';
    localStorage.setItem('lang', lang);
    applyStaticText();
    if (lastLocateStatusKey) document.getElementById('locateStatus').textContent = t(lastLocateStatusKey);
    if (allHospitals.length) render();
  }});

  applyStaticText();

  function haversineKm(lat1, lon1, lat2, lon2) {{
    const R = 6371;
    const dLat = (lat2 - lat1) * Math.PI / 180;
    const dLon = (lon2 - lon1) * Math.PI / 180;
    const a = Math.sin(dLat / 2) ** 2 +
      Math.cos(lat1 * Math.PI / 180) * Math.cos(lat2 * Math.PI / 180) * Math.sin(dLon / 2) ** 2;
    return R * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
  }}

  // hours의 day: 1=월 ... 7=일, 8=공휴일(공휴일 달력까진 반영 안 함 - 평일/토/일만 정확)
  // JS Date.getDay(): 0=일 ... 6=토
  function isOpenNow(hours) {{
    if (!hours || hours.length === 0) return false;
    const now = new Date();
    const jsDay = now.getDay();
    const todayCode = jsDay === 0 ? 7 : jsDay;
    const slot = hours.find(h => h.day === todayCode);
    if (!slot) return false;
    const nowVal = now.getHours() * 100 + now.getMinutes();
    const start = Number(slot.start);
    const end = Number(slot.end);
    return nowVal >= start && nowVal <= end;
  }}

  function fmtHours(h) {{
    return `${{h.slice(0,2)}}:${{h.slice(2)}}`;
  }}

  function todayHoursText(hours) {{
    const now = new Date();
    const jsDay = now.getDay();
    const todayCode = jsDay === 0 ? 7 : jsDay;
    const slot = hours && hours.find(h => h.day === todayCode);
    if (!slot) return t('todayHoursNone');
    return `${{t('todayHoursPrefix')}} ${{fmtHours(slot.start)}} ~ ${{fmtHours(slot.end)}}`;
  }}

  window.openDirections = function (lat, lon, encodedName) {{
    const appUrl = `nmap://route/car?dlat=${{lat}}&dlng=${{lon}}&dname=${{encodedName}}&appname=kidsshowmap.bochul2`;
    const webUrl = `https://map.naver.com/p?title=${{encodedName}}&lat=${{lat}}&lng=${{lon}}`;
    const start = Date.now();
    window.location.href = appUrl;
    setTimeout(() => {{
      if (Date.now() - start < 2000) window.location.href = webUrl;
    }}, 1200);
  }};

  function render() {{
    const container = document.getElementById('listItems');
    // 소아과 탭엔 공식 소아과뿐 아니라 일반의원(가정의학과/내과 등, 이름에 전문과목이
    // 안 붙은 경우 포함)도 같이 보여준다 - "부천연세365의원"처럼 소아청소년과로 등록은
    // 안 돼있어도 실제로 아이를 봐주는 동네의원이 많아서다. 대신 일반의원은 소아과가
    // 아니라는 걸 명확히 배지로 표시해서, 전화로 먼저 확인하고 가게 유도한다.
    let list = activeCat === '소아과'
      ? allHospitals.filter(h => h.category === '소아과' || h.category === '일반의원')
      : allHospitals.filter(h => h.category === activeCat);

    if (activeCat === '소아과' || activeCat === '약국') {{
      list = list.filter(h => isOpenNow(h.hours));
    }}

    list = list.map(h => ({{ ...h, _dist: haversineKm(refPoint[0], refPoint[1], h.lat, h.lon) }}));
    // 데이터 수집 자체는 전국이라(메인 지도와 같은 이유로 넓힘), 화면에서 내 위치
    // 기준 20km를 벗어난 곳은 걸러낸다.
    list = list.filter(h => h._dist <= FIXED_RADIUS_KM);
    // 확실한 소아과를 항상 위로 - 같은 등급 안에서는 거리순. 안 그러면 더 가까운
    // (하지만 소아과인지 불확실한) 일반의원이 확실한 소아과보다 위에 뜰 수 있어서 헷갈림.
    const tierOf = h => h.category === '소아과' ? 0 : (h.category === '일반의원' ? 1 : 2);
    list.sort((a, b) => tierOf(a) - tierOf(b) || a._dist - b._dist);

    if (list.length === 0) {{
      const msgs = {{
        '소아과': t('emptyClinic'),
        '약국': t('emptyPharmacy'),
        '응급실': t('emptyEr'),
      }};
      container.innerHTML = `<div class="empty-msg">${{msgs[activeCat] || t('emptyDefault')}}</div>`;
      return;
    }}

    container.innerHTML = list.map(h => {{
      let badge;
      if (h.category === '소아과') {{
        badge = `<span class="badge open">${{t('badgeClinic')(todayHoursText(h.hours))}}</span>`;
      }} else if (h.category === '일반의원') {{
        badge = `<span class="badge general">${{t('badgeGeneral')(todayHoursText(h.hours))}}</span>`;
      }} else if (h.category === '약국') {{
        badge = `<span class="badge open">${{t('badgePharmacy')(todayHoursText(h.hours))}}</span>`;
      }} else {{
        badge = `<span class="badge er">${{t('badgeEr')}}</span>`;
      }}
      // 처음엔 주소(시/도+시/군/구)도 검색어에 같이 넣었는데, 오히려 정확한 검색이
      // 안 되는 경우가 있어서 이름만으로 검색하도록 되돌렸다.
      const searchUrl = `https://search.naver.com/search.naver?query=${{encodeURIComponent(h.title)}}`;
      return (
        `<div class="h-card">` +
          badge +
          `<h4><a href="${{searchUrl}}" target="_blank" rel="noopener">${{h.title}}</a><span class="search-hint">${{t('searchHint')}}</span></h4>` +
          `<div class="meta">${{h.address}}</div>` +
          (h.telephone ? `<div class="meta">${{h.telephone}}</div>` : '') +
          `<div class="dist">약 ${{h._dist.toFixed(1)}}km</div>` +
          `<div class="btn-row">` +
            (h.telephone ? `<a class="call-btn" href="tel:${{h.telephone}}">${{t('callBtn')}}</a>` : '') +
            `<a class="directions-btn" href="#" onclick="event.preventDefault(); openDirections(${{h.lat}}, ${{h.lon}}, '${{encodeURIComponent(h.title)}}')">${{t('directionsBtn')}}</a>` +
          `</div>` +
        `</div>`
      );
    }}).join('');
  }}

  document.querySelectorAll('.tab-btn').forEach(btn => {{
    btn.addEventListener('click', () => {{
      document.querySelectorAll('.tab-btn').forEach(b => b.classList.toggle('active', b === btn));
      activeCat = btn.dataset.cat;
      render();
    }});
  }});

  // locateStatus는 위치 확인 결과에 따라 나중에 비동기로 채워지는 문구라, 언어를
  // 바꿀 때 다시 그려주려면 마지막으로 어떤 상태였는지 키로 기억해둬야 한다.
  let lastLocateStatusKey = null;
  function setLocateStatus(key) {{
    lastLocateStatusKey = key;
    document.getElementById('locateStatus').textContent = t(key);
  }}

  function goToCurrentLocation(onDone) {{
    if (!navigator.geolocation) {{
      setLocateStatus('noGeoSupport');
      if (onDone) onDone(false);
      return;
    }}
    navigator.geolocation.getCurrentPosition(
      pos => {{
        refPoint = [pos.coords.latitude, pos.coords.longitude];
        setLocateStatus('gotLocation');
        render();
        if (onDone) onDone(true);
      }},
      () => {{
        setLocateStatus('deniedLocation');
        if (onDone) onDone(false);
      }},
      {{ timeout: 5000 }}
    );
  }}

  document.getElementById('locateBtn').addEventListener('click', () => {{
    setLocateStatus('locating');
    goToCurrentLocation();
  }});

  fetch('data/hospitals.json')
    .then(res => res.json())
    .then(data => {{
      allHospitals = data.hospitals;
      render();
      goToCurrentLocation(); // 거부/실패해도 기본값(부천)으로 이미 떠 있으니 별도 처리 없음
    }})
    .catch(() => {{
      document.getElementById('listItems').innerHTML = `<div class="empty-msg">${{t('loadFailed')}}</div>`;
    }});
</script>
{contact_widget_block}
</body>
</html>
"""


def main():
    contact_widget = render_contact_widget(accent_color="#e0507a", page_source="병원")
    html = HTML_TEMPLATE.format(
        ga_measurement_id=GA_MEASUREMENT_ID,
        contact_widget_css=contact_widget["css"],
        contact_widget_block=contact_widget["block"],
    )
    with open(HOSPITAL_OUTPUT_PATH, "w", encoding="utf-8") as f:
        f.write(html)
    print(f"병원 페이지 생성 완료 -> {HOSPITAL_OUTPUT_PATH}")


if __name__ == "__main__":
    main()
