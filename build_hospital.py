...
<div class="topbar">
  ...
  <div class="tab-row">
    <button type="button" class="tab-btn active" data-cat="소아과">🧒 소아과 (지금 열린 곳)</button>
...
<script>
  ...
  function render() {{
    ...
    let list = allHospitals.filter(h => h.category === activeCat);
    ...
    if (list.length === 0) {{
      const msg = activeCat === '소아과'
        ? '지금 문 연 소아과가 없어요 😢<br>응급실 탭을 확인해보세요'
        : '주변에 응급실 정보가 없어요';
    ...
    container.innerHTML = list.map(h => {{
      const badge = activeCat === '소아과'
        ? `<span class="badge open">🟢 진료중 · ${{todayHoursText(h.hours)}}</span>`
        : `<span class="badge er">🚑 응급실</span>`;
      return (
        `<div class="h-card">` +
          badge +
          `<h4>${{h.title}}</h4>` +
