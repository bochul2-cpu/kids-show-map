...
<script>
  ...
  // (maxBounds)에 걸려 clamp되면서 지도가 엉뚱한 위치로 튀는 버그가 있었다(특히 반경
  // 북쪽 경계에 가까운 마커일수록 심함). 지도를 움직이는 대신 InfoWindow의 pixelOffset만
  // 화면 픽셀 단위로 조정해서 팝업만 시각적으로 내려 보이게 하면 지도 위치는 아예 안
  // 건드리니 이 버그 자체가 생길 수 없다.
  function popupPixelOffset(position) {{
    ...
    const popupTopY = pt.y - popupH; // 팝업은 기본적으로 마커 위쪽에 뜬다
    const neededTop = topbarBottom + 12;
    let shiftDown = Math.max(0, neededTop - popupTopY);

    const maxShift = Math.max(0, window.innerHeight - pt.y - 40); // 마커 아래로 너무 안 밀리게 제한
