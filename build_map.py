...
<style>
  ...
  .date-row input[type=date] {{
    ...
    accent-color: #5b4fe0; color: #555; cursor: pointer; background: #fafafa;
  }}
...
<script>
  ...
  // 팝업이 위쪽 필터바에 가리지 않도록, 필터바 실제 높이 + 팝업 예상 높이만큼
  // 클릭한 위치를 화면 아래쪽으로 이동시킨다
  function panForPopup(position) {{
    const filterBar = document.querySelector('.filter-bar');
    const filterBarBottom = filterBar ? filterBar.getBoundingClientRect().bottom : 0;
    const popupH = window.innerWidth <= 480 ? 260 : 430; // 모바일은 CSS로 팝업이 작아짐
    const minMarkerY = filterBarBottom + popupH + 20;
