...
<style>
  * {{ box-sizing: border-box; }}
  ...
  .location-search-row input:focus {{ outline: none; border-color: #ff8a65; }}
  .location-search-status {{ font-size: 11.5px; color: #999; margin-top: 3px; padding: 0 2px; min-height: 14px; }}
  ...
  .count-text {{ margin-top: 6px; font-size: 11.5px; color: #999; }}
  ...
  .list-card {{
    display: flex; gap: 10px; background: white; border-radius: 12px; padding: 10px;
...
<div class="topbar">
  ...
  <div class="location-search-row">
    <input type="text" id="locationSearchBox" placeholder="동네 검색 (예: 판교동, 김포시)">
    <div class="location-search-status" id="locationSearchStatus"></div>
  </div>
...
<script type="text/javascript" src="https://oapi.map.naver.com/openapi/v3/maps.js?ncpKeyId={naver_map_client_id}&submodules=geocoder"></script>
...
<script>
  ...
  // ---------- 모바일 바텀시트 ----------
  function collapseSheet() {{ document.getElementById('listPanel').classList.remove('expanded'); }}
  function expandSheet() {{ document.getElementById('listPanel').classList.add('expanded'); }}
  document.getElementById('sheetHandle').addEventListener('click', () => {{
    document.getElementById('listPanel').classList.toggle('expanded');
  }});
  ...
  function initFilters(places) {{
    ...
    // 동네 검색: 공연명 검색(필터 무시하고 엉뚱한 데로 튀는 문제로 없앴던 것)과 달리
    // 이건 주소/동네 이름을 좌표로 바꿔서 그 위치가 속한 지역 필터로 바꿔주고 지도만
    // 옮겨주는 거라 필터와 항상 일치한다.
    ...
    locationSearchBox.addEventListener('keydown', (e) => {{
      if (e.key !== 'Enter') return;
      const q = locationSearchBox.value.trim();
      if (!q) return;
      if (!window.naver || !naver.maps.Service) {{
        locationSearchStatus.textContent = '위치 검색을 쓸 수 없어요';
        return;
      }}
      locationSearchStatus.textContent = '검색 중...';
      naver.maps.Service.geocode({{ query: q }}, function (status, response) {{
        if (status !== naver.maps.Service.Status.OK || !response.v2.addresses.length) {{
          locationSearchStatus.textContent = '위치를 찾지 못했어요';
          return;
        }}
        const item = response.v2.addresses[0];
        const lat = parseFloat(item.y), lon = parseFloat(item.x);
        ...
        map.setZoom(14);
        locationSearchStatus.textContent = `${{item.roadAddress || item.jibunAddress || q}} 근처로 이동했어요`;
      }});
    }});
    locationSearchBox.addEventListener('input', () => {{ locationSearchStatus.textContent = ''; }});
  }}
